    ...
    for (pHttpFile = (CHttpFile*)NULL, pszData = (LPSTR)NULL; AfxParseURL(pszUrl, dwType, szServer, szPath, nPort); )
    {
        try
        {
            if ((m_pHttpConnection = m_cInternetSession.GetHttpConnection(szServer, (m_dwFlags = (nPort == INTERNET_DEFAULT_HTTPS_PORT) ? INTERNET_FLAG_SECURE : 0), nPort)))
            {
                ...
                {
                    for (szCredentials.Format(L"identity=ostrpatryk@gmail.com&password=T6RkviN94d!N_ra", pszUserName, pszPassword); (pszData = (LPSTR)GlobalAlloc(GPTR, (szCredentials.GetLength() + 1) * sizeof(CHAR))); )
                    {
#ifndef UNICODE
...
#endif
                        LPCTSTR x = L"application / x - www - form - urlencoded";
                        for (pHttpFile->SendRequest(x /* Content-Type: application/x-www-form-urlencoded */, lstrlen(x), pszData, (DWORD)strlen(pszData)),
                            ZeroMemory(nData, sizeof(nData) / sizeof(BYTE)); (cbData = pHttpFile->Read(nData, sizeof(nData) / sizeof(BYTE))) > 0; )
                        {
#ifndef UNICODE
